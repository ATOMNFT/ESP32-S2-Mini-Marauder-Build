...
void MenuFunctions::battery(bool initial)
{
    ...
    #ifdef MARAUDER_TTGO_TDISPLAY  // Added for TTGO TDISPLAY battery monitoring 
      float voltage = battery_obj.readBatteryVoltage();

      static float last_voltage = 0.0;
      if (initial || abs(voltage - last_voltage) > 0.01) {  // Only update if voltage changes > 0.01V or on first run
        ...
        display_obj.tft.setTextColor(the_color, STATUSBAR_COLOR);
        display_obj.tft.fillRect(180, 0, 60, 6, STATUSBAR_COLOR);  // height = 10 for compact bar

        char voltage_str[10];
        snprintf(voltage_str, sizeof(voltage_str), "%.2fV", voltage);

        RESET_FONT;  // If this overrides the text size, move setTextSize(1) AFTER it or modify RESET_FONT

        display_obj.tft.drawRightString(String(voltage_str), 239, 0, 2);  // align to far right edge
      }
    #else
...
void MenuFunctions::updateStatusBar()
{
  ...
  #if defined(MARAUDER_MINI) || defined(MARAUDER_M5STICKC) || defined(MARAUDER_REV_FEATHER)  || defined(MARAUDER_TTGO_TDISPLAY)
    display_obj.tft.setFreeFont(NULL);
  #endif
