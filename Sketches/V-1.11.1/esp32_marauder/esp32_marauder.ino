...
void setup()
{
  ...
  // PSRAM init note:
  // esp_spiram_init() is not available on ESP32-C3 / RISC-V Arduino core builds,
  // which causes an undefined reference link error. Arduino initializes PSRAM
  // automatically when the selected board/core supports it, and later code can
  // still check psramFound() under HAS_PSRAM.
  ...
  // Removed duplicate esp_spiram_init() call for ESP32-C3/RISC-V compatibility.
  ...
  // #ifdef ESP32_C3_SM
  //   digitalWrite(TFT_BL, HIGH); //---------------------------------- Turn on the backlight *For ESP32 C3 Super Mini* (For Display When uncommented) -------------------------------
  // #endif
